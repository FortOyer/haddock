...
renameType t = case t of
  HsForAllTy { hst_tele = tele, hst_body = ltype } -> do
    tele'  <- renameLHsForAllTelescope tele
...
renameLHsForAllTelescope :: LHsForAllTelescope GhcRn -> RnM (LHsForAllTelescope DocNameI)
renameLHsForAllTelescope (L loc tele) = case tele of
  HsForAllVis   x bndrs -> do bndrs' <- mapM renameLTyVarBndr bndrs
                              pure $ L loc $ HsForAllVis x bndrs'
  HsForAllInvis x bndrs -> do bndrs' <- mapM renameLTyVarBndr bndrs
                              pure $ L loc $ HsForAllInvis x bndrs'
